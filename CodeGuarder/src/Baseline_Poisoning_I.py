...
def main(ori_instruction_path, kng_base_path, embed_model_path, poisoning_model_path, vector_db_path, output_path, shot_num, language="all"):
    ...
    # vul_knowledge = [x["buggy"] for x in _tmp_knowledge]
    ...
    if os.path.exists(vector_db_path):
        ...
        vul_vector_db = FAISS.load_local(vul_vector_db_path, poisoning_model.encode, allow_dangerous_deserialization=True)
    else:
        sec_vector_db = build_vector_database(sec_knowledge, retrieve_model)
        sec_vector_db.save_local(sec_vector_db_path)
        vul_vector_db = build_vector_database(vul_knowledge, poisoning_model)
        vul_vector_db.save_local(vul_vector_db_path)
    ...
    for cur_inst in ori_instructions:
        ...
        if similar_codes !=sec_similar_codes:
            VRRC.append(1)
        else:
            VRRC.append(0)
...
def parse_args():
    parser = argparse.ArgumentParser(description="CodeGuarder Parameters")
    ...
    parser.add_argument("--ori_instruction_path", default="./dataset/Instruction.json", help="Path to original instructions")
    parser.add_argument("--kng_base_path", default="./dataset/SecurityKnowledge.json", help="Path to knowledge base")
...
if __name__ == "__main__":
    ...
    # If output_path not provided, generate it based on language
    if not args.output_path:
        args.output_path = f"./datasets/Baseline_poisoning_I_{args.language}.json"
    
    # Generate vector_db_path based on embed_model_path
    vector_db_path = f"./embeddings/kng_poisoning"
