...
def build_vector_database(root_causes, model):
    ...
    # documents = [root_cause["vulnerable_pattern"] for root_cause in root_causes] # debug
...
def find_root_causes(vector_db, query_text, top_k=3):
    """
    在向量数据库中查找与查询文本最相似的代码。
    :param vector_db: FAISS 向量数据库
...
def main(args, root_cause_path, embed_model_path, vector_db_path, broken_instruction_path, poisoned_instructions_path, ori_instruction_path, output_path, root_cause_num=2):
    ...
    for i, cur_inst in enumerate(tqdm(poisoned_instructions)):
        ...
        # final_RCs = select_root_causes(model, cur_sim_RCs, top_k=root_cause_num)
        final_RCs = []
        ...
        # print(new_item["test_case_prompt"])
...
def parse_args():
    parser = argparse.ArgumentParser(description="Vulnerability Pattern Matching Parameters")
    parser.add_argument("--only_vuln", action="store_true", 
                       help="Only break down vulnerability instructions (default: False)")
    ...
    parser.add_argument("--root_cause_path", required=True, 
                       help="Path to root cause data")
    parser.add_argument("--broken_instruction_path", required=True, 
                       help="Path to broken instructions")
    parser.add_argument("--poisoned_instructions_path", required=True, 
                       help="Path to poisoned instructions")
    parser.add_argument("--ori_instruction_path", required=True, 
                       help="Path to instruction responses")
    parser.add_argument("--output_path", 
                       help="Custom output path (optional)")
    parser.add_argument("--root_cause_num", type=int, default=5, 
                       help="Number of root causes (default: 5)")
    parser.add_argument("--root_cause_per_module", type=int, default=2, 
                       help="Root causes per module (default: 2)")
...
if __name__ == "__main__":
    ...
    if not args.output_path:
        args.output_path = "./datasets/def_poisoning_II.json"
