...
for model in "${MODELS[@]}"; do
    ...
    ORI_JSON=$(conda run --no-capture-output -n "${CONDA_ENV_NAME}" python ./src/sec_eval.py --result_path "results/std_ori_${model}.json" | tail -1)
    DEF_JSON=$(conda run --no-capture-output -n "${CONDA_ENV_NAME}" python ./src/sec_eval.py --result_path "results/std_def_${model}.json" | tail -1)
